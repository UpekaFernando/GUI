...
namespace shopManagementsytem2
{
    ...
    public static class ImageHelper
    {
        ...
        public static ImageBrush GetBackgroundImage(string url = null, bool isLoginScreen = true)
        {
            ...
            if (isLoginScreen)
            {
                ...
                try
                {
                    if (File.Exists(desktopPath))
                    {
                        ...
                        // Check if in cache
                        if (ImageCache.TryGetValue(cacheKey, out ImageBrush cachedBrush))
                        {
                            return cachedBrush;
                        }
            ...
            if (ImageCache.TryGetValue(imageUrl, out ImageBrush cachedWebBrush))
            {
                return cachedWebBrush;
            }
        ...
        public static ImageBrush LoadImageFromResource(string resourceName)
        {
            try
            {
                ...
                if (ImageCache.TryGetValue(packUri, out ImageBrush cachedResourceBrush))
                {
                    return cachedResourceBrush;
                }
        ...
        public static ImageBrush GetProfileImage(string url = null)
        {
            ...
            // Check if we already have this image in the cache
            if (ImageCache.TryGetValue(imageUrl, out ImageBrush cachedProfileBrush))
            {
                return cachedProfileBrush;
            }
            ...
            foreach (var path in possiblePaths)
            {
                try
                {
                    if (File.Exists(path))
                    {
                        ...
                        // Check if in cache
                        if (ImageCache.TryGetValue(cacheKey, out ImageBrush cachedLocalBrush))
                        {
                            return cachedLocalBrush;
                        }
